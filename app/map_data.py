...
class WorldMap:
    ...
    def render(self, selected=()):
        ...
        img = self._lut[self.state_arr].astype(np.float32)
        if selected:
...
def read_state_details(path):
    ...
    Only the state-wide buildings are surfaced - per-province entries like
    `1234 = { naval_base = 3 }` are left alone by both the reader and the
    writer, so naval bases survive an edit untouched.
    """
    with open(path, "r", encoding="utf-8-sig", errors="ignore") as f:
        raw = f.read()
    ...
    return {
        ...
        "buildings": {k: num(buildings, k) for k in BUILDING_KEYS},
